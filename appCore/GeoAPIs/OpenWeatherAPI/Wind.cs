...
namespace OpenWeatherAPI
{
	public class Wind
	{
		...
		{
			try { speed = double.Parse(windData.SelectToken("speed").ToString()); } catch {}
			try { degree = double.Parse(windData.SelectToken("deg").ToString()); } catch {}
			try { direction = assignDirection(degree); } catch {}
			try {
