...
//		const string remotePath = @"D:\ANOC Master Tool\"; // HACK: Override update location
		...
		static void loadUI()
		{
			Application.Run(new appCore.MainForm(trayIcon));
		}
...
//			}
			...
			if (!localPath.Contains("\\bin\\Release\\") && !localPath.Contains("\\bin\\Debug")) {// check if tool isn't being ran from IDE, if it is, skip update and dll checks
